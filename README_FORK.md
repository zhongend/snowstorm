# Snowstorm 积木粒子编辑器（Fork 简介）

> 本文件是 Fork 新增内容的说明。原项目的 README 保持原样，请见 [README.md](README.md)。
>
> **在线演示：[zhongend.github.io/snowstorm](https://zhongend.github.io/snowstorm/)**（打开即默认积木模式，无需安装）

本仓库基于 [Snowstorm](https://snowstorm.app/)（原作者 [JannisX11](https://github.com/JannisX11)，Minecraft 基岩版粒子编辑器）二次开发：在完整保留原版编辑能力的基础上，新增了 **Scratch / TurboWarp 风格的积木式图形编程模式**——像拼积木一样"拼"出粒子效果，右侧 3D 预览实时变化，导出的 JSON 可以直接放进 Minecraft 资源包使用。
...
| 🟩 寿命 | 粒子寿命：最大寿命、消亡条件（Molang）、击杀平面、触碰/离开方块消失 |
| 🟧 运动 | 动态（方向/初速度/加速度/阻力）与参数化（用表达式直接控制位置方向） |
| 🟧 旋转 | 动态（初始角度/转速/加速度/阻力）与参数化（表达式直接控制角度） |
| 🟪 外观 | 粒子尺寸、四种材质混合模式、11 种摄像头朝向、方向来源、环境光照 |
...
| 🔴 事件 | 事件定义（生成子粒子/播放声音/运行表达式）+ 发射器与粒子的事件触发（生成时/结束时/时间线/飞行距离） |
| 🟡 曲线 | 线性/Catmull Rom/贝塞尔曲线，节点可编辑，输出变量名可插入任意数值槽 |
| ⬛ 高级 | 初始变量/每帧变量、粒子更新表达式、粒子渲染表达式 |
...
- 新增代码全部集中在 `src/block_editor/`，对原版文件的改动仅 `App.vue` 模式切换与 `import.js` 一处导出
